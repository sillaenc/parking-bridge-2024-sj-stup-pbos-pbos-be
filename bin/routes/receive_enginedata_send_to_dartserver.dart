...
Future<List<dynamic>> receiveEnginedataSendToDartserver(var engineDbaddr,
    var displayDbAddr, var displayDbLPR, DateTime check) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      error.clear();
      for (var element in parkingLotList.where((e) => e.contains('X000'))) {
        ...
        if (parts.length >= 4) {
          ...
          // 예상 형식이 아닐 때 로그를 남기고 싶다면:
          print('Unexpected format: $element');
        }
      }
      // print('receive.dart error list: $error');
      ...
      if (hour != now.hour) {
        ...
        String formattedTime =
            "${oneHourBefore.year.toString().padLeft(4, '0')}-${oneHourBefore.month.toString().padLeft(2, '0')}-${oneHourBefore.day.toString().padLeft(2, '0')} ${oneHourBefore.hour.toString().padLeft(2, '0')}";
      ...
      if (day != now.day) {
        ...
        try {
          ...
          String formattedTime =
              "${oneDayBefore.year.toString().padLeft(4, '0')}-${oneDayBefore.month.toString().padLeft(2, '0')}-${oneDayBefore.day.toString().padLeft(2, '0')}";
